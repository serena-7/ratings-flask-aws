...
def user_detail(user_id):
    ...
    user_ratings = Rating.query.join(Movie, Rating.movie_id == Movie.movie_id).add_columns(
        Movie.title.label("title"), Rating.score.label("score")).filter(Rating.user_id == user.user_id).all()
    user_avg = Rating.query.with_entities(func.avg(Rating.score)).filter(
...
def movie_detail(movie_id):
    ...
    if eye_rating and effective_rating:
        beratement = BERATEMENT_MESSAGES[abs(
            round(eye_rating) - effective_rating)]
    else:
        beratement = None
